...
class Critic:
    ...
    def get_value_estimates(self, output_ids, input_mask, output_mask):
        ...
        """
        attention_mask = logical_or_without_broadcasting(input_mask, output_mask)   # The padding of input and output are set as False
        outputs = self.model(output_ids, attention_mask=attention_mask)
        return outputs.logits.squeeze() # [B, T], 获取每个token的reward